...
class CompileOrderManager:
    ...
    def get_command(
        ...
        """
        Get the hdldepends command string without executing it.

        This is used when hdldepends needs to run in the same shell session
        as Vivado (e.g., inside a Docker container).
        ...
        Returns:
            Command string to execute hdldepends
        """
        # Build command parts
        parts = [
            "hdldepends",
        ...
        parts.append(str(self.hdldepends_config_path))

        # Return as a single command string
        return " ".join(parts)
    ...
    def generate(
        ...
        This method executes hdldepends directly. For Docker-based setups,
        use get_command() instead to get the command string and execute
        it in the appropriate shell context.
        ...
        # Get command string and split into parts for subprocess
        ...
        logger.info(f"Running hdldepends: {command_str}")
