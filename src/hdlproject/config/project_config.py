...
class ProjectConfig:
    ...
    _pydantic_model: Optional[PydanticConfig] = field(
    ...
    def load_from_yaml(
        ...
        config._set_vivado_version(
            vivado_year, vivado_sub, vivado_location, check_if_vivado_version_exists
        )
    ...
    def _resolve_hdldepends_path(
        pydantic_config: PydanticConfig, project_dir: Path, repository_root: Path
    ) -> Path:
        """
    ...
    def resolve_for_operation(self, operation: str, output_directory: Path) -> None:
        ...
        self.resolved_configuration_path = (
            output_directory / "hdlproject_config_resolved.json"
        )
        ...
        logger.info(
            f"Resolved configuration saved to: {self.resolved_configuration_path}"
        )
    ...
    def validate(self, check_vivado: bool = True) -> list[str]:
        ...
        if not self.top_level_file_path:
            errors.append(
                f"Top-level file '{self.top_level_file_name}' not found in repository"
            )
        ...
        if (
            self.resolved_configuration_path
            and not self.resolved_configuration_path.exists()
        ):
            errors.append(
                f"Resolved configuration not found: {self.resolved_configuration_path}"
            )
    ...
    def get_tcl_arguments(
        self, mode: str, operation_paths: OperationPaths, cores: int = 1
    ) -> list[str]:
        ...
        return [
            ...
            str(cores),
            "--config",
            str(self.resolved_configuration_path),
        ]
    ...
    def _find_top_level_file(self, repository_root: Path) -> Optional[Path]:
        ...
        if not found_files:
            logger.warning(
                f"Top-level file '{self.top_level_file_name}' not found in {repository_root}"
            )
            return None

        if len(found_files) > 1:
            logger.warning(
                f"Multiple files found for '{self.top_level_file_name}': {found_files}"
            )
    ...
    def _set_vivado_version(
        ...
        if check_if_vivado_version_exists:
            if not settings_path.exists():
                raise FileNotFoundError(
                    f"Vivado {year}.{sub} not found at {settings_path}"
                )

        self.vivado_version = VivadoVersion(
            year=year, sub=sub, settings_path=settings_path
        )
